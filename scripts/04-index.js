// scripts/04-index.js
// Nous étudions ici l'indexation et la performance — createIndex(), explain('executionStats').
// Notre objectif est de comparer les performances avant et après création d'index.
// Nous reprenons scripts/indexes/create-indexes.js (utilisé par l'app réelle via
// Mongoose), ici réécrit en mongosh pur.
//
...
// ---------- Requête de référence : nous sélectionnons les commandes CONFIRMED d'un restaurant ----------
...
try {
  ...
  print('(nous ne trouvons pas d\'index existant à supprimer — premier passage)');
}
...
// Nous créons un index composé : restaurants par quartier + ouverture
...
// Nous créons un index texte (recherche plein-texte pondérée)
...
// Nous créons un index composé : commandes par statut + restaurant (CRITIQUE — tableau de bord vendeur)
...
// Nous créons un index composé : historique client trié par date
